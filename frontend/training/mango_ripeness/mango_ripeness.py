...
class ImageProcessing:
    ...
    def plot_rgb_with_image(self):
        ...
        # Add RGB values text on image
        ax.text(10, 20, f'Top RGB: {top_rgb}', color='white', fontsize=12, bbox=dict(facecolor='black', alpha=0.5))
        ax.text(10, h // 3 + 20, f'Center RGB: {center_rgb}', color='white', fontsize=12,
                bbox=dict(facecolor='black', alpha=0.5))
        ax.text(10, 2 * h // 3 + 20, f'Bottom RGB: {bottom_rgb}', color='white', fontsize=12,
                bbox=dict(facecolor='black', alpha=0.5))
...
def plot_radar_chart(scores):
    ...
    ax.set_yticklabels([])
    ax.set_xticks(angles[:-1])  # Don't include duplicated label
